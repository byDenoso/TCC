...
def load_dispatch_request(path: Path) -> DispatchRequest:
    ...
    return DispatchRequest(work_id, correlation_id, domain, adapter, source_revision, attempt, args)
...
def run_dispatch_request(request_path: Path, output_path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        failed.update({"status": "FAILED", "error": {"type": type(exc).__name__, "message": str(exc)}})

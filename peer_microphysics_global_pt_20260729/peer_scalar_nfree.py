...
def _scalarize(value):
    if isinstance(value, (list, tuple)):
        return _scalarize(value[0]) if value else value
    if isinstance(value, np.ndarray):
        return _scalarize(value.reshape(-1)[0]) if value.size else value
...
class PEERCambTransfers(CambTransfers):
    def get_can_support_params(self):
        return set(super().get_can_support_params()) | {
            "peer_fede", "peer_zc", "peer_thetai", "peer_n"
        }


class PEERScalarNFree(cobaya_camb.CAMB):
    """CAMB EarlyQuintessence wrapper with a sampled axion-like index n."""
    ...
    def get_can_support_params(self):
        return super().get_can_support_params() + [
            "peer_fede", "peer_zc", "peer_thetai", "peer_n"
        ]

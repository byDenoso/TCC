...
def install_runtime(work: Path, packages: Path) -> None:
    ...
    with tarfile.open(work / "payload" / "official-likelihood-packages-v3.tar.gz", "r:gz") as archive:
        archive.extractall(workspace)
    if not packages.is_dir():
        raise RuntimeError(f"Official packages were not extracted to {packages}")

    wheels = list((work / "validated").rglob("camb-*.whl"))
    databases = list((work / "validated").rglob("Rec_database"))
    developments = list((work / "validated").rglob("Development"))
    if not wheels or not databases or not developments:
    ...
    _replace_directory_link(work / "Rec_database", databases[0])
...
def timed_run(config: Path, logroot: Path, seconds: int, cwd: Path) -> tuple[int, dict]:
    ...
    with (logroot / "stdout.log").open("w") as stdout, (logroot / "stderr.log").open("w") as stderr:
        try:
            process = run(
